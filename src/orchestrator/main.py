...
def _cluster_health_loop() -> None:
    """Periodically check node health, update master resources, and sync Traefik routes."""
    while True:
        try:
            if _cluster_state:
...
def _sync_traefik_routes() -> None:
    """Generate Traefik dynamic config to route remote services through master.

    For services running on worker nodes (not on master), creates Traefik
    file-provider config so master's Traefik can proxy to them.
    """
    ...
    config_path = os.path.join(config_dir, "cluster-routes.yml")
    ...
    try:
        ...
        placements = _cluster_state.get_placements()
        all_nodes = {n.name: n for n in _cluster_state.list_nodes()}

        # Group services by name -> list of node IPs where they run
        svc_nodes = {}
        for p in placements:
            svc = p.service_name
            if not svc or svc in SYSTEM_SERVICES:
                continue
            if svc not in svc_nodes:
                svc_nodes[svc] = set()
            node = all_nodes.get(p.node_name)
            if node and p.status == "running":
                ip = node.address.split(":")[0] if ":" in node.address else node.address
                svc_nodes[svc].add(ip)
        ...
        for svc_name, ips in svc_nodes.items():
            if not ips:
                continue
            # Check if service has containers on master (Traefik Docker provider handles those)
            # Only add file-provider routes for services NOT on master
            master_has_it = False
            for p in placements:
                if p.service_name == svc_name and p.node_name == master_node_name and p.status == "running":
                    master_has_it = True
                    break

            if master_has_it:
                continue  # Docker provider already handles it

            import re
            safe = re.sub(r"[^a-z0-9-]", "-", svc_name.lower()).strip("-") or "svc"
            route_name = f"cluster-{safe}"

            # Router: PathPrefix (no strip - forward as-is to worker Traefik)
            routers[f"{route_name}-path"] = {
                "rule": f"PathPrefix(`/{svc_name}/`)",
                "service": route_name,
                "entryPoints": ["web"],
            }
            # Router: Host
            ...
            # Service: proxy to worker Traefik (port 80) which handles strip+routing
            servers = [{"url": f"http://{ip}:80"} for ip in sorted(ips)]
            services[route_name] = {
    ...
    try:
        # Write as simple YAML manually (no pyyaml dependency needed)
        lines = ["# Auto-generated by AI Container Orchestrator", "http:"]
        ...
        if services:
            ...
            for name, cfg in services.items():
                ...
                for s in cfg["loadBalancer"]["servers"]:
